...
static void libsave_1oom_make_header(uint8_t *buf, const char *savename)
{
    ...
    SET_LE_32(&buf[LIBSAVE_1OOM_OFFS_VERSION], GAME_SAVE_VERSION);
    strncpy((char *)&buf[LIBSAVE_1OOM_OFFS_NAME], savename, SAVE_NAME_LEN);
}

static void *libsave_1oom_open_check_header(const char *filename, int i, bool update_table, char *savename);
...
int libsave_1oom_do_load(const char *filename, struct game_s *g, int savei, char *savename)
{
    FILE *fd = NULL;
    uint8_t *savebuf = NULL;
    int res = -1, len = 0;
...
static void *libsave_1oom_open_check_header(const char *filename, int i, bool update_table, char *savename)
{
    ...
    if (fd) {
        if (1
          ...
          && (GET_LE_32(&hdr[LIBSAVE_1OOM_OFFS_VERSION]) == GAME_SAVE_VERSION)
        ) {
            if (update_table) {
...
int libsave_1oom_read_header(const char *filename, struct libsave_1oom_hdr_data_s *hdr_data)
{
    uint8_t hdr[LIBSAVE_1OOM_HDR_SIZE];
    FILE *fd;
    int res = 0;
    ...
    if (fd) {
        if (1
          ...
          && (memcmp(hdr, (const uint8_t *)LIBSAVE_1OOM_MAGIC, 8) == 0)
            ...
            if (hdr_data != NULL) {
                hdr_data->version = GET_LE_32(&hdr[LIBSAVE_1OOM_OFFS_VERSION]);
                memcpy(hdr_data->savename, &hdr[LIBSAVE_1OOM_OFFS_NAME], SAVE_NAME_LEN);
                hdr_data->savename[SAVE_NAME_LEN - 1] = '\0';
            }
            res = LIBSAVE_1OOM_HDR_SIZE;
        }
        fclose(fd);
        fd = NULL;
    }
...
const char *libsave_1oom_get_slot_fname(int i, char *buf, int buflen)
{
