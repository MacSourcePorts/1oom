...
static inline enum save_cmoo_offs_t save_cmoo_havesave_offs(int i)
{
    if ((i < 0) || (i > 5)) {
...
static inline enum save_cmoo_offs_t save_cmoo_savename_offs(int i)
{
    if ((i < 0) || (i > 5)) {
        return SAVE_CMOO_LEN;
    }
